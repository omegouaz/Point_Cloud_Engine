'''
    fragment a .bag video into a small bits of videos to make it
    eaiser to process the images.
'''
...
from .utility import join, make_clean_folder, get_rgbd_file_lists
from .opencv_utility import initialize_opencv
...
def read_rgbd_image(color_file, depth_file, convert_rgb_to_intensity, config):
    color = o3d.io.read_image(color_file)  # pylint: disable=no-member
    depth = o3d.io.read_image(depth_file)  # pylint: disable=no-member
    ...
    for s in range(sid, eid):
        for t in range(s + 1, eid):
            ...
            if s % config['n_keyframes_per_n_frame'] == 0 \
                    ...
                    if success:
                        pose_graph.edges.append(o3d.registration.PoseGraphEdge(s - sid,
                                                       t - sid,
                                                       trans,
                                                       info,
                                                       uncertain=True))                   
